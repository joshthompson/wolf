...
let fetchUpdates = (timeout?: number) => {
	if (game) {
		timeout = timeout ? timeout : 10000
		request('GET', `/api/game/${game.getId()}`).getBody('utf8').then(JSON.parse).done((g: any) => {
			game = new WolfGame(g)
...
let setupGame = () => {
	...
	// Create new game
	document.getElementById('create-btn').onclick = (event: Event) => {
		game = new WolfGame()
		request('POST', '/api/game', {json: {game: game}}).getBody('utf8').then(JSON.parse).done((g: any) => {
			game.setId(g.id)
	...
	// Join game
	document.getElementById('join-btn').onclick = (event: Event) => {
		...
		request('POST', `/api/game/${code}/join`, {json: {player: player}}).getBody('utf8').then(JSON.parse).done((g: any) => {
			game = new WolfGame(g)
			fetchUpdates()
